...
#+begin_src emacs-lisp
  ...
  (keymap-set lisp-interaction-mode-map "<RET>" 'newline-and-indent)

  ;; (with-eval-after-load 'semantic 
  ;;   (keymap-set semantic-mode-map "C-c j" 'semantic-ia-fast-jump)
  ;;   (keymap-set semantic-mode-map "C-c i" 'semantic-ia-show-summary)
  ;;   (keymap-set semantic-mode-map "C-c d" 'semantic-ia-show-doc))
  ...
  (keymap-global-set "C-c o" 'ff-find-other-file-other-window)

  (keymap-global-set "C-c a a" 'apropos)
  (keymap-global-set "C-c a f" 'apropos-function)
  (keymap-global-set "C-c a v" 'apropos-variable)
  (keymap-global-set "C-c a i" 'info-apropos)

  (keymap-global-set "C-c b m" 'ojm/messages)
  (keymap-global-set "C-c b s" 'ojm/scratch)
  (keymap-global-set "C-c c" 'check-parens)
  (keymap-global-set "C-c l e" 'eshell)
  (keymap-global-set "C-c l t" 'term)
...
#+begin_src emacs-lisp
  ...
  (defun project-gen-tags ()
    "Generate (recursive) Ctags file for Emacs in the current project."
    (interactive)
    (in-project (shell-command "ctags -eR .")))

  (defun project-todo ()
    "Recursive grep for TODO in current project"
    (interactive)
     (project-find-regexp "\\<TODO\\>"))

  (keymap-set project-prefix-map "E" 'project-gen-tags)
  (keymap-set project-prefix-map "T" 'project-todo)
...
#+begin_src emacs-lisp
  (setq message-send-mail-function 'smtpmail-send-it
        ...
        message-mail-alias-type 'ecomplete ;; add sent-to addresses to a etion db
...
#+begin_src emacs-lisp
  (defun configure-fonts ()
    (set-face-attribute 'default nil :font "IosevkaTermNerdFont" :height 90 :width 'narrow)
    ;; (set-face-attribute 'default nil :font "Terminus")
    (set-face-attribute 'fixed-pitch nil :font "IosevkaTermNerdFont" :height 90 :width 'narrow)
...
Apply Flyspell to yanked text.
#+begin_src emacs-lisp
  (let ((flyspell-yank-excluded-modes
         '(erc-mode)))
    (defun ojm/flyspell-yanked (&rest args)
      (when (and flyspell-mode
                 (not (seq-find (lambda (m) (derived-mode-p m)) 
                                flyspell-yank-excluded-modes)))
        (flyspell-region (region-beginning) (region-end)))))

  (advice-add 'yank :after 'ojm/flyspell-yanked)
  (advice-add 'yank-pop :after 'ojm/flyspell-yanked)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'eglot
    ...
    (keymap-set eglot-mode-map "C-c F" 'flymake-show-project-diagnostics)
    (keymap-set eglot-mode-map "C-c f" 'flymake-show-buffer-diagnostics)
    ...
    (keymap-set eglot-mode-map "C-c e o" 'eglot-code-action-organize-imports)
